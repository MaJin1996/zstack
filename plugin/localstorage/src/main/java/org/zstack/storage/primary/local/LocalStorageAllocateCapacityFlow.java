    ...
    private String getRequiredStorageUuid(String hostUuid, String psUuid){
        if(psUuid == null){
            return getMostFreeLocalStorageUuid(hostUuid);
                ...
                .eq(PrimaryStorageVO_.uuid, psUuid).isExists()){
            throw new OperationFailureException(argerr("the type of primary storage[uuid:%s] cheesed is not local storage, " +
                    "check if the resource can be created on other storage when cluster has attached local primary storage"));
        }else if(Q.New(LocalStorageHostRefVO.class)
                .eq(LocalStorageHostRefVO_.primaryStorageUuid, psUuid)
                .eq(LocalStorageHostRefVO_.hostUuid, hostUuid).isExists()){
            return psUuid;
        }else {
            return getMostFreeLocalStorageUuid(hostUuid);
        }
